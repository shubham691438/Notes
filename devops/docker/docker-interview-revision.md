# Docker — Interview Revision Notes (Java / Spring Boot)

> Short on time? Read §1 → §5 → §7 → §11 → §16.
...
**Container vs VM** (asked in almost every interview):
- A **VM** has **its own kernel** and a full OS. Size in GB, boots in minutes, very strong isolation.
- A **container shares the host kernel**. It is isolated using Linux **namespaces** (pid, net, mnt, uts, ipc, user), **cgroups** (CPU/memory limits), and a **union filesystem** (overlay2).
- Size in MB, starts in milliseconds, weaker isolation (all containers share one kernel).
- On Mac and Windows there *is* a small Linux VM underneath, because containers are a Linux feature.
...
```
docker CLI  --REST API-->  dockerd  -->  containerd  -->  runc  -->  your process
                              |
                              +--> registry (pull / push)
```
...
- **BuildKit** — the new build engine (default now). Parallel stages, cache mounts, secret mounts.
...
- `docker run` = pull the image if it's missing + create the container (writable layer, network) + start it. Your process becomes **PID 1**.
...
| Command | What it does |
|---|---|
| `docker run -d --name app -p 8080:8080 myapp:1.0` | Creates and starts a container. `-d` = run in background, `--name` = give it a name, `-p host:container` = open the port. |
...
| `docker logs -f --tail 100 app` | Shows the last 100 lines and keeps following new ones (like `tail -f`). |
| `docker exec -it app sh` | Opens a shell **inside** a running container. The normal way to look around. `-it` = interactive terminal. |
| `docker inspect app` | Prints all details as JSON: env vars, mounts, network, exit code, whether it was OOM-killed. |
...
| Command | What it does |
|---|---|
| `docker volume create pgdata` | Creates a named volume — a folder managed by Docker. |
...
| Command | What it does |
|---|---|
| `docker network create app-net` | Creates a user-defined network. Containers on it can find each other **by name**. |
...
### 4.7 Step by step: build an app, a database, a network and a volume
...
**Step 2 — create a network** so the app and the database can find each other by name.
```bash
docker network create app-net
```

**Step 3 — create a volume** so the database data survives when the container is deleted.
```bash
docker volume create pgdata
```

**Step 4 — start Postgres** on that network, with that volume.
```bash
...
- `-p 5432:5432` — only needed if you also want to connect from your Mac (DBeaver, IntelliJ). Containers on `app-net` don't need it.

**Step 5 — check it is ready**
```bash
...
**Step 6 — start your app on the same network**
```bash
...
docker logs -f app
```
Note the URL uses `postgres` — the **container name** — and port **5432**, the port *inside* the network. Not `localhost`.

**Step 7 — prove the volume works**
```bash
...
```
Then use these (psql commands start with a backslash, SQL ends with a semicolon):
...
| `\dt` | **List the tables** |
| `\d users` | Show the columns, types and indexes of the `users` table |
| `\dn` | List schemas |
| `\du` | List users / roles |
...
If you see "Did not find any relations", you are usually in the wrong database or the wrong schema. Check with `\c appdb` and then `\dt public.*`.
...
> Passwords typed on the command line are fine for local practice, but not for real environments — see §12.

---

## 5. Dockerfile — the rules that matter
...
| `COPY` vs `ADD` | COPY only copies files. ADD can also download URLs and unpack tar files. Use COPY — it is simpler and predictable. |
| `EXPOSE` vs `-p 8080:8080` | EXPOSE is just information. `-p host:container` is what really opens the port. `-P` opens all EXPOSEd ports on random host ports. |
| `ARG` vs `ENV` | ARG works only during the build (`--build-arg`). ENV stays in the image and is visible in `docker inspect` and image history — so keep secrets out of both. |

**Layer cache — the most common build question**
...
⚠️ **Trap:** deleting a file in a later layer does **not** make the image smaller — the file is still inside the earlier layer and can be pulled out. Install and clean up in the **same** `RUN`. Also, if your Dockerfile does `COPY target/*.jar`, don't put `target/` in `.dockerignore`.

---

## 6. Images: size, tags, cleanup

- **Multi-stage build** — use the full JDK + Maven in stage 1, then `COPY --from=builder` only the JAR into a small JRE stage. Build tools, source code and the Maven cache never reach the final image. A Spring Boot image usually drops from ~800 MB to ~200 MB on a JRE base, and under 100 MB with alpine/jlink.
- **Base image options:** `21-jdk` (big) → `21-jre` → `21-jre-alpine` (uses musl libc — watch out for native libraries and DNS issues) → distroless (no shell, smallest attack surface, hardest to debug).
...
- A **dangling image** is the old untagged layer set left behind when you rebuild the same tag (`<none>:<none>`). Clean it with `docker image prune`.

---

## 7. ⭐ Spring Boot + Docker (most of your questions will be here)
...
### 7.2 Layered JARs (Spring Boot 2.3+)
Spring Boot writes a `layers.idx` file that splits the JAR into 4 layers, from "rarely changes" to "changes every time":
1. `dependencies`
2. `spring-boot-loader`
3. `snapshot-dependencies`
4. `application` — your code and resources (the only one that changes in a normal deploy)
...
### 7.3 The Dockerfile you should be able to write from memory
...
### 7.4 Buildpacks — no Dockerfile at all
```bash
mvn spring-boot:build-image -Dspring-boot.build-image.imageName=myorg/myapp:1.0
./gradlew bootBuildImage
```
- Uses **Cloud Native Buildpacks (Paketo)**. Needs Docker running. You get a good JRE, layers, container-aware JVM memory settings and a non-root user for free.
- **Use it when** you don't want to maintain Dockerfiles across many services.
- **Use a Dockerfile when** you need extra OS packages, a company base image, or full control.

### 7.5 The JVM inside a container (common senior question)
...
⚠️ Getting this wrong is the usual reason for **exit code 137**. If someone asks "why does my pod keep restarting under load?", this is the answer they want.
...
```
⚠️ A Docker **restart policy does not look at the healthcheck**. An unhealthy container keeps running; only the process exiting causes a restart. Kubernetes *does* restart on a failed liveness probe, and Swarm reschedules. Also `curl` is often missing in slim images — use `wget -qO-`.
...
| **user-defined bridge** | Same, plus **built-in DNS** — containers can reach each other **by name**. Always prefer this. |
...
⚠️ The **default** bridge has **no DNS**. That is the classic "my app cannot find `mysql`" bug. Compose creates a user-defined network for you, so service names work: `jdbc:mysql://mysql:3306/appdb`.
...
- Plain `depends_on: [mysql]` only controls **start order**. It does not wait until MySQL is ready — that is why the app crashes on startup. Fix it with `condition: service_healthy`, and also add retry in the app, because a good service should not depend on start order.
...
## 11. Running, exit codes and debugging
...
- **Logs:** Docker collects stdout/stderr of PID 1. So log to the console, never to a file inside the container. Logging drivers (`json-file`, `local`, `awslogs`, `fluentd`, `gelf`) send them somewhere else. Always set `max-size`/`max-file` on `json-file` or the disk fills up.
...
- Use a small, pinned base image. Rebuild often for security patches. Scan with `docker scout cves`, Trivy or Snyk.
...
- Use `--read-only` for the root filesystem plus a tmpfs for `/tmp`; `--cap-drop ALL --cap-add NET_BIND_SERVICE`; `--security-opt no-new-privileges`.
- Never mount `/var/run/docker.sock` into a container you don't trust — that gives root on the host.
- Use rootless Docker on shared machines. Sign your images and keep internal ones in a private registry.
...
⚠️ **Common on Mac:** an M-series Mac builds `linux/arm64`, but most servers are `linux/amd64` → you get `exec format error`. Fix it with `docker buildx build --platform linux/amd64,linux/arm64 --push`.
...
## 15. Mistakes people make in interviews

1. Saying "containers are small VMs" — they are isolated processes sharing the host kernel.
2. Thinking `EXPOSE` opens a port. It doesn't.
3. Thinking deleting a file in a later layer makes the image smaller.
4. Thinking `latest` means newest. It's just a tag.
5. Thinking `depends_on` waits until the DB is ready.
6. Thinking restart policies react to healthchecks.
7. Using shell-form ENTRYPOINT, which breaks SIGTERM and graceful shutdown.
8. Mixing up heap size and container memory (exit 137).
9. Forgetting the default bridge network has no DNS by name.
10. Running `docker rm` with no volume and losing the database.
11. Putting secrets in `ENV`, where anyone who pulls the image can read them.
12. Building on Apple Silicon and deploying to amd64 without buildx.

---

## 16. Quick questions and answers (say them out loud)

**Basics**
- *What is Docker and why use it?* → It packages the app with everything it needs into one image, so it behaves the same everywhere, starts fast, and uses fewer resources than VMs.
- *Image vs container?* → A read-only template vs a running copy of it with a writable layer. Class vs object.
- *Container vs VM?* → Shared kernel with namespaces and cgroups vs its own kernel. MB and milliseconds vs GB and minutes. Weaker vs stronger isolation.
- *What happens on `docker run`?* → Pull the image if missing, create the container with a writable layer and network, then start the entrypoint as PID 1.
- *Docker architecture?* → CLI → dockerd (REST API) → containerd → runc, plus a registry for images.

**Images / Dockerfile**
- *What is a layer?* → A read-only set of file changes made by one Dockerfile line, cached and shared between images.
- *How do you make builds faster?* → Put rarely-changing lines first, copy `pom.xml` and download dependencies before `src`, use `.dockerignore`, BuildKit cache mounts and multi-stage builds.
- *How do you make the image smaller?* → Multi-stage build, JRE or distroless base instead of JDK, install and clean up in one RUN, `.dockerignore`. Find the big layer with `docker history`.
- *CMD vs ENTRYPOINT?* → ENTRYPOINT is the command, CMD is the default arguments. Args after `docker run` replace CMD; `--entrypoint` replaces ENTRYPOINT.
- *What is a dangling image?* → Old untagged `<none>` layers left after rebuilding a tag. Remove with `docker image prune`.

**Storage / network**
- *Volume vs bind mount?* → A folder managed by Docker (portable, easy to back up) vs a specific host path. Volumes for DB data, bind mounts for dev and config.
- *How do two containers talk?* → On the same user-defined network, using the service or container name (Docker's built-in DNS) and the container port.
- *Bridge vs host vs overlay?* → Private network on one host / share the host's network / span many hosts.

**Compose**
- *Compose vs Dockerfile?* → A Dockerfile builds one image; Compose runs and connects many containers.
- *The app starts before the DB is ready — how do you fix it?* → Add a `healthcheck` on the DB and `depends_on: condition: service_healthy`, and add retry in the app (HikariCP `initializationFailTimeout`, Flyway retries).

**Spring Boot**
- *How do you dockerize a Spring Boot app?* → Multi-stage build: Maven builds the JAR, `jarmode=tools extract --layers` splits it, copy the 4 layers into a small JRE image, run as non-root, use exec-form ENTRYPOINT, and pass config through env vars. Or use `mvn spring-boot:build-image` and skip the Dockerfile.
- *Why layered jars?* → Dependencies rarely change and your code changes often, so only the small `application` layer is rebuilt, pushed and pulled.
- *How do you set JVM memory in a container?* → `UseContainerSupport` (on by default since JDK 10) reads the container limits; set `MaxRAMPercentage=75` and leave room for metaspace, threads and direct buffers, or you get exit 137.
- *How do you handle profiles and config?* → `SPRING_PROFILES_ACTIVE` and env vars: one image, different config per environment, secrets passed at runtime.
- *How do you get a clean shutdown?* → `server.shutdown=graceful` plus exec-form ENTRYPOINT so the JVM is PID 1 and gets SIGTERM within the 10 second window.
- *How do you expose health?* → Actuator liveness and readiness endpoints, a Docker `HEALTHCHECK`, and in Kubernetes the liveness/readiness/startup probes.
- *How do you test against a real database?* → Testcontainers with `@ServiceConnection`; `spring-boot-docker-compose` for local development.

**Security**
- *How do you secure a container?* → Non-root user, small pinned base image, regular scanning, no secrets in the image, read-only filesystem, dropped capabilities, never mount the Docker socket, rootless daemon.
- *How do you handle secrets?* → Only at runtime: env from a secret manager, Docker or Kubernetes secrets, Vault, or BuildKit secret mounts. Never inside a layer.

---

## 17. Debugging playbook

Scenario questions are checking whether you have a method. Say the steps out loud.

**"The container exits immediately."**
`docker ps -a` for the exit code → `docker logs` → `docker inspect`. Usual reasons: the app failed at startup (wrong datasource URL, missing env var), CMD was not a foreground process, wrong path or entrypoint (127), or out of memory (137). Reproduce with `docker run -it --entrypoint sh image` and run the command by hand.
...
**"My code change didn't appear after rebuild."**
Either the layer cache is stale or you are running the old tag. Try `docker build --no-cache`, check the image ID, use `docker compose up --build`, and make sure `.dockerignore` isn't excluding your source.

**"The image is 1.2 GB."**
`docker history` to find the big layer, then multi-stage build + JRE base + cleanup in the same RUN.

**"The app can't reach MySQL."**
...
1. Dockerize one Spring Boot service two ways — your own multi-stage Dockerfile and `spring-boot:build-image`. Compare image size and rebuild time after a one-line change.
2. Run `docker history` on your image, find the biggest layer, and remove it.
3. Compose: app + MySQL + healthcheck with `depends_on`. Stop the DB container and see what the app does. Add a named volume and check the data survives `docker compose down` and disappears with `down -v`.
4. Run with `--memory=512m` and default JVM settings, put load on it, get exit 137, then fix it with `MaxRAMPercentage`.
5. Break things on purpose: use shell-form ENTRYPOINT and time `docker stop`; use the default bridge and watch DNS fail.
6. Try `docker buildx build --platform linux/amd64,linux/arm64`.
